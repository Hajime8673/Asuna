...
class RetryHandler:
    def __init__(self, on_error=True, on_match=[], on_unmatch=[], on_type_match=[], on_type_unmatch=[],
                 retries=3, retry_delay=0, delay_multiply=0, handler=None):
        """
        Initializes RetryHandler class with specified parameters.
    ...
    def retry(self, func: Callable[..., Any]) -> Callable[..., Any]:
        """
        Decorator function to handle retries of a given function.
            ...
            for attempt in range(self.retries + 1):
                try:
                    ...
                    if (not self.on_match or result not in self.on_match) and \
                       ...
                       (not self.handler or self.handler(result)):
                        return result
                ...
                if attempt < self.retries and self.retry_delay > 0 and not attempt+1 == self.retries:
                    time.sleep(self.retry_delay + (attempt * self.retry_delay * self.delay_multiply))
